...
export async function buildAgentAuthHeaders(config, method, path) {
  ...
  const message = `paygrid:erc8004:${config.agentId}:${account.address}:${normalizedMethod}:${path}:${timestamp}:${nonce}`;
...
export async function paygridRequest(config, path, init = {}, options = {}) {
  ...
  if (options.agentAuth) {
    Object.assign(headers, await buildAgentAuthHeaders(config, method, path));
  }
  ...
  if (!response.ok) {
    const message = body?.message ?? body?.error ?? `Paygrid API returned ${response.status}`;
